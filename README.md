...
- **Integrations.** Optional upload to HeyPocket or Omi after processing, or Save to Folder, which copies recordings into a folder on the phone or SD card without leaving the device (in the recording's own format, and counted like any integration when recordings are deleted after upload — the local audio goes only once the folder has its copy). Each integration has its own queue and uploads one recording at a time, but different integrations upload concurrently — a slow Omi upload no longer blocks HeyPocket. Omi uploads are split into ~5-minute chunks sent one at a time, with live chunk-level progress and resume-on-retry. Per-integration status (Queued / Uploading / Uploaded / Failed — Saving / Saved for Save to Folder) is shown per recording with individual retry actions, plus per-day "Upload All" and multi-select "Upload Selected" batch actions.
