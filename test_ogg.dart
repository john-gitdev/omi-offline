...
// Include the VadAudioProcessor code to generate a dummy OGG file
// I will just use a minimal version to generate the OGG and test it with opusinfo
...
Uint8List _createOggPage(int granulePos, int seqNum, int serial, List<Uint8List> packets,
  ...
  for (final p in packets) {
    segmentCount += (p.length ~/ 255) + 1; // FIX: using ~/ instead of .floor() just to be pure dart
  }
  ...
  final pageBytesList = <int>[];
  pageBytesList.addAll(header.buffer.asUint8List());
  for (final p in packets) {
    pageBytesList.addAll(p);
  }

  final pageBytes = Uint8List.fromList(pageBytesList);
...
Uint8List _createOggOpusIdHeader() {
  ...
  header.setUint8(1, 0x70);
  header.setUint8(2, 0x75);
  header.setUint8(3, 0x73);
  header.setUint8(4, 0x48);
  header.setUint8(5, 0x65);
  header.setUint8(6, 0x61);
  header.setUint8(7, 0x64);
  header.setUint8(8, 1);
  header.setUint8(9, 1); // Channels
  header.setUint16(10, 0, Endian.little); // Pre-skip
  header.setUint32(12, 16000, Endian.little); // Sample rate
  header.setUint16(16, 0, Endian.little);
  header.setUint8(18, 0);
  return header.buffer.asUint8List();
}

Uint8List _createOggOpusCommentHeader() {
  final vendorBytes = [0x4f, 0x6d, 0x69]; // 'Omi'
  final header = ByteData(8 + 4 + vendorBytes.length + 4);
  header.setUint8(0, 0x4f);
  header.setUint8(1, 0x70);
  header.setUint8(2, 0x75);
  header.setUint8(3, 0x73);
  header.setUint8(4, 0x54);
  header.setUint8(5, 0x61);
  header.setUint8(6, 0x67);
  header.setUint8(7, 0x73);
  header.setUint32(8, vendorBytes.length, Endian.little);
  final list = header.buffer.asUint8List();
  list.setAll(12, vendorBytes);
  ByteData.view(list.buffer).setUint32(12 + vendorBytes.length, 0, Endian.little);
  return list;
}

void main() async {
  final f = File("test.ogg");
  ...
  // Add one silence frame
  sink.add(_createOggPage(960, 2, serial, [Uint8List.fromList([0xF8, 0xFF, 0xFE])], isLastPage: true));
