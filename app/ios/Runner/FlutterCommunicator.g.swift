...
protocol WatchRecorderHostAPI {
  ...
  func sendAudioSegment(audioSegment: FlutterStandardTypedData, segmentIndex: Int64, isLast: Bool, sampleRate: Double) throws
...
class WatchRecorderHostAPISetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: WatchRecorderHostAPI?, messageChannelSuffix: String = "") {
    ...
    let sendAudioSegmentChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.watch.WatchRecorderHostAPI.sendAudioSegment\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      sendAudioSegmentChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let audioSegmentArg = args[0] as! FlutterStandardTypedData
        let segmentIndexArg = args[1] as! Int64
        let isLastArg = args[2] as! Bool
        let sampleRateArg = args[3] as! Double
        do {
          try api.sendAudioSegment(audioSegment: audioSegmentArg, segmentIndex: segmentIndexArg, isLast: isLastArg, sampleRate: sampleRateArg)
          reply(wrapResult(nil))
        } catch {
          reply(wrapError(error))
        }
      ...
      sendAudioSegmentChannel.setMessageHandler(nil)
    }
...
protocol WatchRecorderFlutterAPIProtocol {
  ...
  func onAudioSegment(audioSegment audioSegmentArg: FlutterStandardTypedData, segmentIndex segmentIndexArg: Int64, isLast isLastArg: Bool, sampleRate sampleRateArg: Double, completion: @escaping (Result<Void, PigeonError>) -> Void)
...
class WatchRecorderFlutterAPI: WatchRecorderFlutterAPIProtocol {
  ...
  func onAudioSegment(audioSegment audioSegmentArg: FlutterStandardTypedData, segmentIndex segmentIndexArg: Int64, isLast isLastArg: Bool, sampleRate sampleRateArg: Double, completion: @escaping (Result<Void, PigeonError>) -> Void) {
    let channelName: String = "dev.flutter.pigeon.watch.WatchRecorderFlutterAPI.onAudioSegment\(messageChannelSuffix)"
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
    channel.sendMessage([audioSegmentArg, segmentIndexArg, isLastArg, sampleRateArg] as [Any?]) { response in
      guard let listResponse = response as? [Any?] else {
        completion(.failure(createConnectionError(withChannelName: channelName)))
        return
      }
