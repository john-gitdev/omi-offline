...
void main() {
  ...
  group('OmiDeviceConnection.performGetAudioCodec caching', () {
    const codecUuid = OmiDeviceConnection.featuresCodecCharacteristicUuid;
    ...
    test('a failed read falls back to pcm8 without latching it', () async {
      // A transient GATT rejection on a live link — observed three times in the
      // 2026-08-17 log. The old code cached the fallback, so the device stayed
      // mislabelled until the next disconnect (the only thing that clears the cache).
      transport.throwReads.add(codecUuid);
      expect(await connection.performGetAudioCodec(), BleAudioCodec.pcm8);

      // Same connection, read now succeeds: the real codec must win.
      transport.throwReads.remove(codecUuid);
      transport.reads[codecUuid] = [21];
      expect(await connection.performGetAudioCodec(), BleAudioCodec.opusFS320);
    });

    test('an empty response is treated as no answer, not as pcm8', () async {
      transport.reads[codecUuid] = [];
      expect(await connection.performGetAudioCodec(), BleAudioCodec.pcm8);

      transport.reads[codecUuid] = [20];
      expect(await connection.performGetAudioCodec(), BleAudioCodec.opus);
    });

    test('a successful read is cached — including an unrecognised codec byte', () async {
      // An unknown byte is a real answer ("not opus"), so it must latch: re-reading
      // a value that cannot change is the redundant BLE traffic the cache exists for.
      transport.reads[codecUuid] = [99];
      expect(await connection.performGetAudioCodec(), BleAudioCodec.pcm8);

      transport.reads[codecUuid] = [21];
      expect(await connection.performGetAudioCodec(), BleAudioCodec.pcm8, reason: 'must be served from cache');
    });
  });
}
