...
class WaveformUtils {
    ...
    switch (wavInfo.bitsPerSample) {
      ...
      case 24:
        for (int i = 0; i < pcmData.length - 2; i += 3) {
          int sample =
              pcmData[i] | (pcmData[i + 1] << 8) | (pcmData[i + 2] << 16);
  ...
  static WavInfo? _parseWavHeader(Uint8List wavData) {
    ...
    // ⚡ Bolt: Pass data directly and use positional arguments to avoid sublist copy
    ...
    // ⚡ Bolt: Hoist a single ByteData view outside the loop to avoid allocations
    final wavBd = ByteData.sublistView(wavData);

    while (offset < wavData.length - 8) {
      // ⚡ Bolt: Use positional arguments for String.fromCharCodes
    ...
    offset = 12;
    while (offset < wavData.length - 8) {
      // ⚡ Bolt: Use positional arguments and hoisted ByteData view
      final chunkId = String.fromCharCodes(wavData, offset, offset + 4);
      final chunkSize = wavBd.getUint32(offset + 4, Endian.little);
