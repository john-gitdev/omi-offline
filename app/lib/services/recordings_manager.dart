...
class RecordingsManager {
  ...
  static Future<void> cleanUpIncompleteExtraction() async {
    ...
    Logger.debug(
      'RecordingsManager: Detected incomplete extraction from previous run — cleaning up temp dir.',
    );
    try {
  ...
  Future<List<Batch>> getBatches() async {
    ...
    if (await rawSegmentsDir.exists()) {
      final deviceSessionFolders = rawSegmentsDir
          .listSync()
          .whereType<Directory>()
          .toList();
    ...
    for (var dateStr in allDates) {
      ...
      raw.sort((a, b) {
        ...
        final prefixCmp = nameA
            .split('_')
            .first
            .compareTo(nameB.split('_').first);
    ...
    try {
      ...
      Future<void> moveTempFilesToLive() async {
        ...
        for (final entity in entities) {
          ...
          final millis = parts.length >= 2
              ? int.tryParse(parts.last.split('.').first)
              : null;
          final dateStr = (millis != null && millis > 0)
      ...
      try {
        final processor = await VadAudioProcessor.create(
          outputDir: tempProcessingPath,
        );
        try {
          for (int i = 0; i < allSegments.length; i++) {
            final file = allSegments[i];
            final pathStr = file.path;
            final startIdx = pathStr.lastIndexOf('/') + 1;
            final dotIdx = pathStr.indexOf('.', startIdx);
            int endIdx = dotIdx == -1 ? pathStr.length : dotIdx;
            final underscoreIdx = pathStr.indexOf('_', startIdx);
            if (underscoreIdx != -1 && underscoreIdx < endIdx) {
              endIdx = underscoreIdx;
            }
            final timerStart = int.tryParse(
              pathStr.substring(startIdx, endIdx),
            );
            const kMinValidEpoch = 946684800;
            final segmentStartTime =
                timerStart != null && timerStart > kMinValidEpoch
                ? DateTime.fromMillisecondsSinceEpoch(timerStart * 1000)
                : file.lastModifiedSync();

            if (_cancelRequested) {
              Logger.debug(
                "RecordingsManager: Processing cancelled at segment $i.",
              );
              break;
            }
      ...
      if (lastSafeToDeleteIndex >= 0 &&
        ...
        for (int i = 0; i <= lastSafeToDeleteIndex; i++) {
          ...
          if (await file.exists()) {
            Logger.debug(
              "RecordingsManager: Deleting completed raw segment: ${file.path}",
            );
            await file.delete();
  ...
  static Future<void> _resolveMarkerConversations(
    ...
    final recordings =
        <({File file, int startMs, int endMs, int durationMs})>[];
    ...
    for (final markerTime in markerTimestamps) {
      ...
      if (matchIdx >= 0) {
        ...
        final markerOffsetMs = markerMs < rec.endMs
            ? markerMs - rec.startMs
            : rec.durationMs;
        final edlData = {
    ...
    final resultsNested = await Future.wait(
      dateFolders.map((dateFolder) async {
        ...
        final markerFutures = edlFiles.map((edlFile) async {
          try {
            final json =
                jsonDecode(await edlFile.readAsString())
