...
class RecordingsManager {
      ...
      if (allSegments.isNotEmpty) {
        ...
        if (effectiveVadEnabled) {
          try {
            ...
            Logger.error('RecordingsManager: Failed to pre-load VAD model ($e) — AAD mode active.');
          }
        }
  ...
  Future<List<MarkerConversation>> getMarkerConversations() async {
    ...
    for (final dateFolder in dateFolders) {
      ...
      // ⚡ Bolt: Execute I/O operations and jsonDecode concurrently to prevent N+1 sequential await overhead
      await Future.wait(
        edlFiles.map((edlFile) async {
          try {
            ...
            if (segmentFilename != null && segmentFilename.isNotEmpty) {
              if (segmentFilename.contains('_draft.')) return;
