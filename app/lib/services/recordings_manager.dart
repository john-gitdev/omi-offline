...
Future<void> _processingIsolateEntry(_IsolateParams params) async {
  ...
  try {
    for (int i = 0; i < params.segmentPaths.length; i++) {
      ...
      fileStopwatch.stop();
      Logger.debug(
        'RecordingsManager isolate: segment ${i + 1}/${params.segmentPaths.length} '
    ...
    Logger.debug(
      'RecordingsManager isolate: processing run finished — '
...
class RecordingsManager {
  ...
  Future<void> deleteDay(Batch batch, {bool onlyReprocessable = false}) async {
    ...
    if (!onlyReprocessable) {
      ...
      int deletedBins = 0;
      // ⚡ Bolt Optimization: Batch file IO deletions concurrently and eliminate exists() system call
      await Future.wait(
        batch.rawSegments.map((bin) async {
          try {
            await bin.delete();
            deletedBins++;
      ...
      // ⚡ Bolt Optimization: Batch session dir empty checks concurrently
      await Future.wait(
        touchedSessionDirs.map((dir) async {
          try {
      ...
      if (deletedBins > 0) {
        Logger.debug('RecordingsManager: Deleted $deletedBins raw bins for ${batch.dateString}');
      }
    ...
    // ⚡ Bolt Optimization: Batch surgical file deletions concurrently and eliminate exists() system call
    await Future.wait(
      allToProcess.map((conv) async {
