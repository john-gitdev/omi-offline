...
class VadAudioProcessor {
  ...
  Future<String?> _saveRecording(List<FrameRef> refs, DateTime startTime, {bool? isDerivedTimestamp}) async {
    ...
    Logger.debug('VadAudioProcessor: Saved recording (${refs.length} frames, ${((totalSamples * 1000) ~/ sampleRate)}ms) '
  ...
  Future<String?> _saveOgg(List<FrameRef> refs, String dateFolderPath, int timestamp, {String prefix = 'recording'}) async {
    ...
    try {
      ...
      for (var i = 0; i < refs.length; i++) {
        ...
        if (pcmData != null) {
          ...
          totalSamples += pcmData.length;
          granulePos += pcmData.length;
        }

        final isLastPage = (i == refs.length - 1);
        // Ogg Opus granule position is at 48kHz. Since our PCM is 16kHz, we multiply by 3.
        sink.add(_createOggPage(granulePos * 3, pageSeqNum++, serial, [opusBytes], isLastPage: isLastPage));
      }
      ...
      await sink.close();
      try {
        if (await oggFile.exists()) await oggFile.delete();
      } catch (_) {}
      return await _saveWav(refs, dateFolderPath, timestamp, prefix: prefix);
    }
  }
  ...
  static const List<int> _crcTable = [
    ...
    0xd727bbb6, 0xd3e6a601, 0xdea580d8, 0xda649d6f, 0xc423cd6a, 0xc0e2d0dd, 0xcd11f604, 0xc9d0ebb3,
  ...
  Future<String> _saveWav(List<FrameRef> refs, String dateFolderPath, int timestamp,
    ...
    // Note: _saveWav is a fallback, but we should still save metadata so it appears in the list.
    // Calculate simple peaks for the WAV file (can be improved if needed)
    final dynamicPeaks = List<double>.filled(200, 0.1);

    await _saveMetadata(refs, dateFolderPath, timestamp, totalPcmBytes ~/ (channels * 2), dynamicPeaks, 200,
