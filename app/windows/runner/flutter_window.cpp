...
bool FlutterWindow::OnCreate() {
  ...
  overlay_channel_->SetMethodCallHandler(
      [this](const flutter::MethodCall<flutter::EncodableValue> &call,
             std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>>
                 result) { HandleOverlayMethodCall(call, std::move(result)); });
  ...
  system_tray_->onQuit = [this]() { PostQuitMessage(0); };
...
void FlutterWindow::HandleOverlayMethodCall(
  ...
  if (method == "showOverlay") {
    ...
    if (arguments) {
      auto isRecording_it =
          arguments->find(flutter::EncodableValue("isRecording"));
      auto isPaused_it = arguments->find(flutter::EncodableValue("isPaused"));

      if (isRecording_it != arguments->end() &&
          isPaused_it != arguments->end()) {
    ...
    if (arguments) {
      auto transcript_it =
          arguments->find(flutter::EncodableValue("transcript"));
      auto segmentCount_it =
          arguments->find(flutter::EncodableValue("segmentCount"));

      if (transcript_it != arguments->end() &&
          segmentCount_it != arguments->end()) {
...
void FlutterWindow::ShowOverlay() {
  ...
  std::cout << "FlutterWindow::ShowOverlay() - Main window state: visible="
            << isVisible << ", minimized=" << isIconic << std::endl;

  // Always try to create overlay fresh when needed
  std::cout << "FlutterWindow::ShowOverlay() - Creating fresh overlay..."
            << std::endl;
  floating_overlay_ = std::make_unique<FloatingOverlay>();

  if (floating_overlay_->Create()) {
    std::cout << "FlutterWindow::ShowOverlay() - Overlay created successfully, "
    ...
    floating_overlay_->onPlayPause = [this]() {
      ...
      if (overlay_channel_)
        overlay_channel_->InvokeMethod("onPlayPause", nullptr);
    };
    floating_overlay_->onStop = [this]() {
      std::cout << "FlutterWindow: onStop callback triggered" << std::endl;
      if (overlay_channel_)
        overlay_channel_->InvokeMethod("onStop", nullptr);
    };
    floating_overlay_->onExpand = [this]() {
      std::cout << "FlutterWindow: onExpand callback triggered" << std::endl;
      BringAppToFront();
      if (overlay_channel_)
        overlay_channel_->InvokeMethod("onExpand", nullptr);
    };

    std::cout
        << "FlutterWindow::ShowOverlay() - Callbacks set, showing overlay..."
        << std::endl;
    floating_overlay_->Show();
  } else {
    std::cerr << "FlutterWindow::ShowOverlay() - Failed to create overlay"
              << std::endl;
  }
}
...
void FlutterWindow::UpdateOverlayTranscript(const std::string &transcript,
                                            int segmentCount) {
  if (floating_overlay_) {
  ...
  if (system_tray_ && segmentCount > 0) {
    std::string status =
        "Recording (" + std::to_string(segmentCount) + " segments)";
