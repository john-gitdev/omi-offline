...
| `sco_active` / `audio_active` | Is audio actually flowing: a voice call routed to Bluetooth / anything playing | The contention that matters. A **classic** SCO call is the worst case (reserved slots the controller cannot preempt) — but an LE Audio headset on a call sets `sco_active` too and claims no SCO slot, reserving scheduled isochronous events instead, so read it as "a call was on Bluetooth", not "SCO" specifically. `audio_active` is not BT-specific, so it implies A2DP streaming only alongside an `a2dp` entry above |
