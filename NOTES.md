...
2. **Idle-gc during AAD silence** (worker idle tick, **not** the pause handler): AAD only pauses SD writes when no audio is being ingested, so during a pause the worker's write-wait loop times out with both queues drained — a genuine idle window. It runs `lfs_fs_gc` there (allocator pre-warm only; `compact_thresh = -1` disables compaction) to refill the lookahead **off the write path**. The gate is deliberately conservative. The load-bearing guard is **`worst_full_scan_ms ≤ IDLE_GC_MAX_SCAN_MS` (5 s)**: `lfs_fs_gc` is a single *un-abortable* atomic scan, so anything that arrives while it runs — a phone connecting, a sync being admitted, speech resuming — waits for the whole thing regardless of the start-time guards. Only running scans we've *observed* to be short (seeded from the boot pre-warm, then maxed by each refill; re-baselined per boot) bounds that wait to ~5 s, well under any BLE command timeout — which is what actually makes the race safe. Long scans on a full card are deliberately **not** pre-empted (left to the queue + lookahead + prompt sync to keep the FS emptier); a long scan is the one too costly to hide in a gap. The rest are start-time filters that make the mid-scan-race *rare*, not impossible: `!ble_connected` (don't start while connected — biases the work to when nobody's around, and aims it at devices actually filling the card); `sd_write_paused` (silence; also 0 during force-capture, which AAD never pauses); `bytes_written_since_gc ≥ IDLE_GC_BYTES_THRESHOLD` (half the window); **both msgqs empty**; `!is_storage_sync_active()` (defense-in-depth for the disconnect edge); the pause persisted `≥ IDLE_GC_MIN_SILENCE_MS` (10 s, on top of the ~10 s VAD-hold); and `≥ IDLE_GC_RETRY_COOLDOWN_MS` (60 s) since the last *attempt*, so a persistently failing `lfs_fs_gc` can't re-scan on every timeout. Resumed speech mid-scan queues (120-deep, ~10 s) and drops only if the scan outlasts the queue — the same loss the mid-write scan would cause anyway, now much rarer; on a card full enough for a 50 s scan, keeping it emptier (prompt sync+delete) is the real lever. **Deliberately not in `REQ_PAUSE_IO`:** that handler ACKs the AAD thread, which blocks on it (`k_sem_take`, 10 s), so a multi-second gc there would freeze AAD wake/resume handling — the exact pause/wake interaction behind the priority-marker-loss history. In the idle tick nothing waits on the worker. A write/marker arriving mid-gc queues (120-deep) and is served when gc returns. **Effective in auto/AAD mode only** — manual mode records continuously with no silence window, so there the scan is unavoidably mid-write and the levers are lookahead size + queue depth + keeping the FS emptier (scan cost ∝ data on disk → prompt app-side sync+delete is the biggest lever on *duration*).
...
**READ + NOTIFY** (notify added `oo-2.6.1`: while a client is subscribed the firmware pushes the payload every 2 s during an active transfer / 15 s idle heartbeat, stopping on unsubscribe/disconnect — see "Live diagnostics during sync"). Returns **84 bytes, little-endian**, twenty-one `u32` fields (as of `oo-2.6.2`). The 84-byte notify needs ATT MTU ≥ 87; on a link that never negotiated up (default 23 B) `bt_gatt_notify` returns `-EMSGSIZE` and the push is dropped (firmware `LOG_DBG`s it, doesn't retry). The plain **READ** is the MTU-agnostic fallback — ATT read-blob fragments across PDUs, so the full 84 B is always readable regardless of MTU. In practice `CONFIG_BT_L2CAP_TX_MTU=498` + `AUTO_UPDATE_MTU` make a sub-87 MTU the rare exception. **App gap (Part-1 subscribe lifecycle, tracked separately):** the Sync-page watchdog re-subscribes on notify silence but does not currently issue that READ fallback, so on a chronically small MTU the card would stay stale — a read fallback there must be gated on "not mid-transfer" to avoid the Error-133 read/stream race the notify design exists to dodge. Fields were appended over time — length grew 20 → 28 → 32 → 40 → 44 → 60 → 68 → 76 → 84 B — so an older app reads a shorter prefix and ignores the rest; the firmware always returns the full current length. Offsets past 36 (`estab_fail_count` @40 … `codec_stack_used` @80) are enumerated in the `transport.c` `diagnostics_drops_pack` header comment and the CLAUDE.md BLE table; the layout below documents the original first 40 B in detail.
