...
Read-only. Returns **84 bytes, little-endian**, twenty-one `u32` fields (as of `oo-2.6.2`). Fields were appended over time — length grew 20 → 28 → 32 → 40 → 44 → 60 → 68 → 76 → 84 B — so an older app reads a shorter prefix and ignores the rest; the firmware always returns the full current length. Offsets past 36 (`estab_fail_count` @40 … `codec_stack_used` @80) are enumerated in the `transport.c` `diagnostics_drops_pack` header comment and the CLAUDE.md BLE table; the layout below documents the original first 40 B in detail.
