...
- **New: Per-integration upload status.**
    - With two or more integrations configured, the day-list cloud icon now shows one aggregate icon (colored by the worst state needing attention) with a small badge counting how many integrations apply. Tapping it — in the list or the player — opens a detail sheet with a row per integration showing its own state (Uploaded / Pending / Failed / Uploading / Not available) and a per-integration Upload / Retry / Re-upload action, plus "Upload all pending". You can now retry just the one integration that failed instead of re-uploading to all of them. Single-integration setups are unchanged.
