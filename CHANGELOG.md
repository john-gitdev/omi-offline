...
- **New: "Companion Device Pairing" toggle (Android, App Settings).** An advanced switch for Bluetooth connection trouble. On by default — the app uses Android's system companion association (kept for the background-run grant / "companion" status; it no longer arms presence observation). If your phone keeps a hidden system link to the Omi that competes with the app for the connection — the OnePlus/Oppo/Realme "toggle Bluetooth to reconnect" behavior — turn it off: the app then connects purely by address, and any existing association is cleared on the next connection. Re-pair the device to turn it back on.
...
- **New: Clean slate when flashing over non-Omi-offline firmware.** The first boot after flashing onto a device that was running a different (e.g. stock Omi) firmware wipes the SD card storage **and** any inherited Bluetooth bonds, so recordings and pairing both start fresh. The trigger is the on-card filesystem itself — the NAND is reformatted whenever it isn't recognized as an Omi-offline filesystem (every Omi-offline format stamps a magic cookie) — so this runs exactly once on migration. Upgrading between Omi-offline versions keeps the cookie and therefore **never** wipes storage or bonds (unlike the old per-version wipe removed in oo-1.9).
