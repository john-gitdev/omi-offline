...
- **The foreground notification no longer makes any sound or vibration.** The channel now explicitly sets `setSound(null, null)` and `enableVibration(false)`, and each notification is built with `.setSilent(true)` and `.setOnlyAlertOnce(true)`.
...
- **Foreground notification no longer makes sound on every sync status update.** The Android notification channel was configured with `priority: HIGH` and `onlyAlertOnce: false`, causing a sound every time the notification text was updated during a sync or processing run. Priority lowered to `LOW` and `onlyAlertOnce: true` added.
