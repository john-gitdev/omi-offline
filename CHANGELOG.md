...
- **Turning off an integration now stops its uploads immediately.** Switching off either the **Enabled** or the **Auto-Upload** toggle for Omi Cloud / HeyPocket cancels that integration's in-progress upload and clears everything queued for it. Previously, turning off Auto-Upload left in-progress and queued uploads running, and turning off Enabled only dropped the queue while the active upload kept going. A chunked Omi Cloud upload stops cleanly between chunks (already-delivered chunks are kept and the rest resume if you switch it back on); a cancelled upload is not counted as a failure. (A HeyPocket upload already mid-request finishes, since a single request can't be interrupted once sent.)
