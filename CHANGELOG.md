...
- **Cancel a queued upload from the recording's Integrations list.** A recording waiting its turn to upload now shows a **Cancel** button instead of a static "Waiting…". If it's the only thing in that integration's queue, tapping Cancel just removes it; if a queue is backed up behind it, you're asked whether to cancel only that recording or clear the whole queue. (An auto-upload you cancel this way may come back on the next auto sweep while Auto-Upload is still on; a manual one stays cancelled.)
