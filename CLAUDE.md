...
|---------|-------------|---------|
| Settings | `0010` / `0011`–`0016` | `0011` LED dim ratio, `0012` mic gain, `0013` VAD threshold, `0014` Priority Recording safety cap (u16 LE minutes, `0`=no cap), `0015` button config (6 B), `0016` haptic config (6 B). Button + haptic config were consolidated here from the retired `23ba7926` service (app 0.26.x); byte layouts unchanged, requires a re-pair. |
...
| Features | `0020` (service) / `0021` / `0022` | `0021` capability bitfield (see `OmiFeatures`: accelerometer, button, battery, usb, haptic, offlineStorage, ledDimming, micGain, vadThreshold, priorityRecordCap, `recordToggle` (`1<<11`, AAD-gated) — the app hides the "Single recording button" switch / Toggle action on firmware without it); `0022` audio codec ID read. |
| Time sync | `0030` / `0031` | Write epoch seconds (u32 LE) |
| Battery detail | `0050` / `0051` | Notify 1 byte: `charging` (0/1) |
| Diagnostics | `0060` / `0061` / `0062` | `0061` 8 B: `reset_cause u32 LE` + `uptime_seconds u32 LE`. `0062` 68 B drop counters (seventeen u32 LE): `blockDrops` + `lastDropUptimeMs` + `sdStreamDrops` + `sdBootDrops` + `nowUptimeMs` + `connFails` + `lastFailedAdvSlow` + `codecDrops` + `msgqPeakDepth` + `writeFairActivations` + `estabFailCount` + `priorityRecordStarts` + `priorityRecordStops` + `markerWriteDrops` + `emptyBinRotations` + `sessionEndMarkerEmits`(off 60) + `markerPauseGateSaves`(off 64). Append-only; older apps read a prefix (length grew 20→28→32→40→44→60→68 B). Full layout in NOTES.md "SD Write Drop Counters". |
...
| `0x18` | ARM_POST_DFU_UNPAIR | `[0x18, arm]` (`arm`=1 arm / 0 disarm; bare `[0x18]` arms). Persists a one-shot NVS flag (`omi/unpair_on_boot`); consumed on the next boot in `transport_start` (after BT bonds load), but `bt_unpair` runs **only when the firmware version changed** since the last boot (tracked in `omi/boot_fw_version`) — so a failed/aborted flash that leaves the same image running can't wipe bonds on a later ordinary reboot (or the Reboot command). Armed pre-flash by the app's "Reset pairing after update" toggle (`sendArmPostDfuUnpair`); the app gates its phone-side `removeBond` (on success) on the arm write landing. |
