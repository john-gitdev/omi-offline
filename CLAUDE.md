...
|---------|-------------|---------|
| Settings | `0010` / `0011`–`0017` | `0011` LED dim ratio, `0012` mic gain, `0013` VAD threshold, `0014` Priority Recording safety cap (u16 LE minutes, `0`=no cap), `0015` button config (6 B), `0016` haptic config (6 B), `0017` connected-LED enable (1 B, `0`=off / non-zero=on, default on). Button + haptic config were consolidated here from the retired `23ba7926` service (app 0.26.x); byte layouts unchanged, requires a re-pair. |
