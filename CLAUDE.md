...
- **GATT-cache refresh (Android)**: Android persists a bonded device's attribute database and answers discovery from it, so a firmware flash that adds/moves attributes leaves a stale map (new services invisible; shifted handles misaddressed). `DeviceProvider._shouldRefreshGattCache` fingerprints the device as `<DIS firmware revision>|<capability bitfield>` (`gattFingerprint_<deviceId>` pref) and, when it changes, sets `_needsGattCacheRefresh`; `_onDeviceConnected` then calls `recycleConnection()` **after** clearing `_currentlySettingUpId` (recycling re-enters `_onDeviceConnected`, which would early-return and leave the new link unconfigured). The reconnect path already calls `closeGatt()` → `BluetoothGatt.refresh()`, so no native change is needed. Both halves of the fingerprint matter: version alone misses a dev↔production swap at the same version, which differ in attribute count via `CONFIG_OMI_DIAG_LOG`. Skipped on iOS (no `refresh()`; relies on Service Changed), on a `features == 0` read (degraded link), on first sighting (record only), and while a sync is in flight (deferred to the next connect by not persisting).
