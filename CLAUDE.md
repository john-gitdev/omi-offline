...
| `0x16` | REBOOT | `[0x16]` (deferred to the storage thread: ACKs, gracefully closes the SD card (`app_sd_off()`, guarded like the power-off path), then `sys_reboot(SYS_REBOOT_COLD)`; `sendRebootCommand`. Surfaced as "Reboot Omi" in Device Settings) |
