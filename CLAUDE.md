...
- The firmware writes sequential bin files, rotating at `FILE_ROTATION_INTERVAL_MS` = **10 minutes** (`sd_card.h`; the neighbouring `MAX_AUDIO_FILES` comment still says "5-min rotation" and is wrong). Explicit rotations — app command, BLE connect, priority start/stop, manual stop — make real files shorter than that. Each file's `segmentStartTime` (the WAL `timerStart`) picks up exactly where the previous file ended — no overlaps, no gaps larger than clock drift.
...
| Features | `0020` (service) / `0021` / `0022` | `0021` capability bitfield (see `OmiFeatures`: accelerometer, button, battery, usb, haptic, offlineStorage, ledDimming, micGain, vadThreshold, priorityRecordCap, `recordToggle` (`1<<11`, AAD-gated) — the app hides the "Single recording button" switch / Toggle action on firmware without it; `diagLog` (`1<<12`, set only under `CONFIG_OMI_DIAG_LOG`) — gates the Debug Tools event-log toggle; `ledService` (`1<<13`) — the LED service `0080` exists; gates the Customization "LEDs On" + "Connected LED" switches); `0022` audio codec ID read. |
| Time sync | `0030` / `0031` | Write epoch seconds (u32 LE) |
| Battery detail | `0050` / `0051` | Notify 1 byte: `charging` (0/1) |
| Diagnostics | `0060` / `0061` / `0062` | `0061` 8 B: `reset_cause u32 LE` + `uptime_seconds u32 LE`. `0062` 96 B drop counters (twenty-four u32 LE): `blockDrops` + `lastDropUptimeMs` + `sdStreamDrops` + `sdBootDrops` + `nowUptimeMs` + `connFails` + `lastFailedAdvSlow` + `codecDrops` + `msgqPeakDepth` + `writeFairActivations` + `estabFailCount` + `priorityRecordStarts` + `priorityRecordStops` + `markerWriteDrops` + `emptyBinRotations` + `sessionEndMarkerEmits`(off 60) + `markerPauseGateSaves`(off 64) + `sdWorkerStackUsed`(off 68) + `codecStackUsed`(off 72) + `ringMaxIoMs`(off 76) + `ringIoErrors`(off 80) + `lastMicFrameUptimeMs`(off 84) + `vadVoicedMs`(off 88) + `advModes`(off 92). Append-only; older apps read a prefix (length grew 20→28→32→40→44→60→68→76→84→92→96 B). `advModes` is `[active u8][desired u8]`, 0=fast/1=slow — the **live** advertising interval, which nothing reported before. Do not confuse it with `lastFailedAdvSlow`(off 24), the mode during the last *failed* connection; that one was twice misread as the current mode during review. Advertising stops while connected, so the value the app reads is the interval in force when the phone found the device. The last two are the mic's own health: `lastMicFrameUptimeMs` against `nowUptimeMs` says whether the mic is delivering **right now** (frames land every 100 ms, so seconds of silence means parked or stopped) — the question that otherwise has to be inferred from the *absence* of event-log records, which is wrong whenever the mic is legitimately parked; `vadVoicedMs` against `nowUptimeMs` is the capture duty cycle, i.e. what the auto threshold actually costs in encode + NAND writes. The advertising watchdog reports through the `0063` event log (`DIAG_ADV_START_FAIL`, `DIAG_ADV_WATCHDOG_RESCUE`) rather than a counter here — see BLE_Research.md, Wedge 5. `emptyBinRotations`(off 56) is **not** a loss signal and was read as one for a long time. An empty bin means *nothing at all* reached the card for that segment, and an **accepted** marker cannot be the missing part — `write_marker_header_to_storage()` force-drains its partial 440 B block, so a marker the SD queue takes puts the bin well past header-only. (Rejected, that block stays in `transport.c`'s assembly buffer, which no rotation drains — `sd_msgq` holds formed blocks, not that buffer — so the marker is not lost but lands in the *next* bin, leaving this one empty. Needs a full SD queue, and bumps no counter of its own.) What is left is a rotation that landed where nothing was being written, which in auto mode is any silent stretch (the age rotation is only evaluated when a write arrives, so an idle device never trips it; an explicit rotate does). Two Force Syncs over a quiet lunch break move it and mean nothing. `markerWriteDrops`(off 52) is the loss signal and stands alone — do **not** read the two together, since both are boot-cumulative totals naming no segment, so a marker dropped in one recording and an empty bin from an idle rotation an hour later look exactly like a correlated loss. Per-bin cause and timing belong to the `0063` event log. Full layout in NOTES.md "SD Write Drop Counters". `0063`/`0064` (dev builds only, `CONFIG_OMI_DIAG_LOG`) = the on-device diagnostic **event log**: `0063` drain read returns a 12 B snapshot header `[record_size u8][reserved u8][record_count u16][dropped_count u32][max_seq u32]` + N×16 B packed `diag_event_t` records `[seq u32][uptime_ms u32][code u8][backend u8][arg0 u16][arg1 u32]`; `0064` control write `[enable u8][ack_seq u32 LE]` (5 B, fail-closed on short write) sets the runtime gate and drops records with `seq ≤ ack_seq`. The ring is volatile RAM (reclaimed from `SD_WORKER_STACK_SIZE`), keep-newest on overflow, drained+acked on connect when the app pref `diagLogEnabled` is on. Records timestamp/contextualize the same health events the `0062` counters only total (empty-bin rotation, marker drop, pause-gate save, priority start/stop, session-end, block/codec drop). `DIAG_EMPTY_BIN_ROTATION` (1) carries the rotation's cause in `arg0` (`rotate_reason_t` in `sd_card.h`: 1 mount, 2 bin age, 3 BLE connect, 4 app rotate command, 5 priority start, 6 priority stop, 7 time sync, 8 storage cleared, 9 active bin deleted, 10 manual/session-end stop, 11 idle BLE connect) and the closing size in `arg1`; only reasons 6 and 10 are worth a second look — each closes a bin that should hold its recording's own markers, so an empty one means they never reached the card (dropped, or left buffered by a full queue and carried into the next bin) — and `0`, the firmware's own unknown and what every build before `oo-3.0.2` sent, must render as unattributed rather than as a cause. Two records are **forced past the runtime gate** (`diag_log_event_forced`): both `DIAG_BOND_STATE` emits, because they fire in `transport_start()` seconds before the app connects and opens the gate — the reboot that produces them is the same reboot that closes the gate, so they were discarded on every boot. `DIAG_VAD_LEVEL` (16) is the AAD input level as a 5-minute peak-hold, emitted only on a silent↔non-silent transition or hourly (a per-window emit would evict the whole 128-slot ring in ~10 h, including the forced boot records); `arg0` = window peak, `arg1` = `[floor u16 high][threshold u16 low]`, and a peak of 0 means digital silence from the mic, which a quiet room never produces. `DIAG_MIC_POWER_CYCLE` (17) records the first-boot-of-a-new-version PDM_EN cycle, `arg0`=1 only if the rail was really cycled (reserved — nothing drives PDM_EN since `oo-2.8.5`). `DIAG_MIC_STATE` (18) records the mic gate opening/closing capture — `arg0` = 0 parked / 1 resumed / 2 resume-failed / 3 resumed-but-silent, `arg1` = the threshold that decided it. It exists because a parked mic emits **nothing at all** (no frames means no `DIAG_VAD_LEVEL` window ever closes), so without the transition itself on the record a gap in the log cannot be told apart from a dead mic. The `backend` byte in every record is now vestigial — one storage backend since `oo-2.9.0`, so the firmware always reports 0 and the app no longer renders it. Firmware `diag_log.c`/`diag_log.h`; app `diag_log_record.dart`. |
