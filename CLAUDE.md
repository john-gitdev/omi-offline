...
- **Neither screen may outlive the re-pair.** The pairing request comes from the system dialog, so it is accepted with no tap on either app screen — meaning both have to react to `DeviceProvider.isConnected` on their own or they sit there instructing the user to redo what they just did. The success screen watches it through a `Consumer` scoped to that section (`_buildSuccessSection`) and, when connected, swaps the re-pair card for `_buildReconnectedNotice()` and makes Done skip the `FindDevicesPage` push. `FindDevicesPage` closes itself on connect (`_onDeviceProviderChanged` → `_closePage`), which is safe because every route into it — the drawer item, the recordings-page bluetooth icon, the Connect Omi button — is gated on being disconnected, so a connected user on that page is on a page they could not have opened. Two things to preserve there: `_closePage` no-ops unless the page's route `isCurrent`, since a tap-initiated connect stacks a loading dialog on top and popping would take that instead — the tap path sets `_closing` and pops both itself; and the post-frame `_onDeviceProviderChanged()` in `didChangeDependencies` covers a page opened onto an already-live link, which the listener alone would leave until some unrelated notification arrived. The per-row pairing mark on that page (green check = connected, grey = bonded) comes from `BleHostApi.isDeviceBonded` — `BluetoothDevice.bondState`, a local read with no radio traffic — and **not** from `SharedPreferencesUtil().btDevice`, which still names the device after a DFU wipes the bond and would therefore claim a pairing that no longer exists in exactly the situation the mark exists for.
