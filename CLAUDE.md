...
| Button | `23ba7924-0000-1000-7450-346eac492e92` / trigger char `…7925` | Registered but **never used in either direction**. The firmware notifies no tap events (`transport_notify_button_state` was removed 2026-08-06 — it had zero callers) and the app no longer subscribes or reads (`getBleButtonListener` / `performGetButtonState` removed with it). Button behavior is driven entirely by the firmware FSM + inline audio-stream markers (`0xFFFFFFFE` marker, `0xFFFFFFF8` priority-start, etc.), which is what a recorder built to run disconnected needs — a BLE tap event would only arrive while the phone happened to be connected. **The service registration stays** because removing it shifts the handles of every service after it and costs a re-pair. Button **config** moved to the Settings service (`0015`/`0016`) — see the Omi services table above. |
